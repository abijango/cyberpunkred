...
mod tests {
    ...
    /// Construct a fresh `GigState` for use in hire-flow tests.
    /// `GigState` does not derive `Default` (per WP-604); this helper
    /// supplies sensible empty starting state.
    fn fresh_gig_state() -> GigState {
        use crate::ids::{BeatId, GigId};
        GigState {
            gig: GigId::from("test_gig"),
            current_beat: BeatId::from("hook_1"),
            history: Vec::new(),
